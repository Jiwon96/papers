...
def create_vocabs(domain, maxlen=0, vocab_size=0):
...
def read_dataset(domain, phase, vocab, maxlen):
    ...
    print ("<num> hit rate: {:.2f}, <unk> hit rate: {:.2f}".format(100*num_hit/total, 100*unk_hit/total))
    return data_x, maxlen_x


# data 받아서 padding 함
def word_padding(data, max_len):
    temp =[]
    for item in data:
        temp.append(np.pad(item, (0, max_len), mode= 'constant',constant_values=0)[:max_len].tolist())
...
def get_data(domain, vocab_size=0, maxlen=0):
    ...
    vocab = create_vocabs(domain, maxlen, vocab_size)
